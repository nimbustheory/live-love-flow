          ...
          {c.salesCards.map((card, i) => {
            ...
            return (
              <div key={i} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: "18px 16px" }}>
                ...
                {isAdminCard && (
                  <button onClick={() => setIsAdmin(true)} style={{ display: "block", width: "100%", marginTop: 14, padding: "10px 0", borderRadius: 8, border: "none", background: accent, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", letterSpacing: "0.02em" }}>Open Admin</button>
                )}
              </div>
            );
          })}
